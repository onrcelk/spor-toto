...
def normalize_odds(odds: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized = {k: v / overround for k, v in raw.items()}
    return {
...
class StaticOddsAdapter(OddsAdapter):
    """Read-only adapter for local/provider-normalized rows, including screenshots."""

    def __init__(self, rows: Iterable[dict[str, Any]], source: str = "static_odds") -> None:
        self.source = source
    ...
    def retrieve(self, match_id: str, context: dict[str, Any]) -> RetrievalResult:
        ...
        if str(match_id) in self._duplicates:
        ...
        evidence = Evidence.create(
            match_id, claim, "odds", self.source,
