...
def _team_summary(frame: pd.DataFrame, team: str, before: pd.Timestamp, last_n: int) -> dict[str, Any]:
    eligible = frame[frame["kickoff_iso"] < before].copy().sort_values("kickoff_iso").tail(last_n)
    matches = []
    points = 0
    for _, row in eligible.iterrows():
        is_home = str(row["home_team"]).casefold() == team.casefold()
        result = str(row["actual_1x2"])
...
def build_next_week_report(
    ...
    report_matches = []
    for item in matches:
        kickoff = pd.to_datetime(f"{item.get('date_text','')} {item.get('time_text','')}", dayfirst=True, utc=True, errors="coerce")
        if pd.isna(kickoff):
    ...
    target = Path(output).expanduser(); target.parent.mkdir(parents=True, exist_ok=True); target.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return payload
